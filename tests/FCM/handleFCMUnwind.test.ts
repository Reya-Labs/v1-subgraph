...
import { handleFCMUnwind } from '../../src/mappings/FCM/index';
import { handleIrsInstanceDeployed } from '../../src/mappings/factory';

import { FullyCollateralisedUnwind as aaveFullyCollateralisedUnwind } from '../../generated/templates/aaveFCM/aaveFCM';
import { FullyCollateralisedUnwind as compoundFullyCollateralisedUnwind } from '../../generated/templates/compoundFCM/compoundFCM';
...
describe('handleFCMUnwind()', () => {
  ...
  test('aaveFCM: Initialisation and Accumulation Check', () => {
    // aUSDC
    ...
    const fullyCollateralisedUnwindEvent = changetype<aaveFullyCollateralisedUnwind>(newMockEvent());
  ...
  test('compoundFCM: Initialisation and Accumulation Check', () => {
    // cDAI
    const irsInstanceEvent = createIrsInstanceEvent(Address.fromString('0x6b175474e89094c44da98b954eedeac495271d0f'), 2, 18);
    handleIrsInstanceDeployed(irsInstanceEvent);

    // eslint-disable-next-line no-undef
    const fullyCollateralisedUnwindEvent = changetype<compoundFullyCollateralisedUnwind>(newMockEvent());
    const eventAddress = irsInstanceEvent.params.fcm;
    const trader = randomAddresses[4];
    const desiredNotional = BigInt.fromI32(123456);
    const sqrtPriceLimitX96 = BigInt.fromI32(321);
